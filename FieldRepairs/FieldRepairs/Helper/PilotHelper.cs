    ...
    public static class PilotHelper
    {
        ...
        public static void ApplyPilotHealthDamage(AbstractActor target, WeaponHitInfo hitInfo, int headHits, out string tooltipText)
        {
            ...
            if (target.GetPilot().BonusHealth > 0)
            {
                ...
                Mod.Log.LogDebug($"Bonus health aborbs: {absorbedDamage} leaving: {healthDamage} healthDamage.");
            ...
            {
                Mod.Log.LogDebug($"Health damage: {healthDamage} would kill pilot, reducing to maxHealth: {target.GetPilot().Health} - 1");
            ...
            if (!target.UnitIsTrooperSquad() || !Mod.Config.DisableInjuriesForTroopers)
            {
                if (healthDamage > 0)
                {
                    ...
                    Mod.Log.LogDebug($"Adding {healthDamage} to {target.DisplayName}");
                    target.GetPilot().StatCollection.ModifyStat<int>(hitInfo.attackerId, hitInfo.stackItemUID,
        ...
        public static void ApplyPilotSkillDamage(AbstractActor target, WeaponHitInfo hitInfo, int skillDamageHits, out string tooltipText)
        {
            ...
            Mod.Log.LogDebug($"Applying {skillDamageHits} hits to pilot skills to: {target.DisplayName}");
            ...
            Mod.Log.LogDebug($"  A total penalty of -{combinedPenalty} will be applied to all pilot skills");
            ...
            Mod.Log.LogDebug($"  reducing piloting: -{pilotingMod}  gunnery: -{gunneryMod}  tactics: -{tacticsMod}  guts: -{gutsMod}"); ;
