    ...
    public static class PilotHelper
    {
        ...
        public static void ApplyPilotHealthDamage(AbstractActor target, WeaponHitInfo hitInfo, int headHits, out string tooltipText)
        {
            ...
            if (target.GetPilot().BonusHealth > 0)
            {
                ...
                Mod.Log.Info?.Write($"Bonus health aborbs: {absorbedDamage} leaving: {healthDamage} healthDamage.");
            ...
            {
                Mod.Log.Info?.Write($"Health damage: {healthDamage} would kill pilot, reducing to maxHealth: {target.GetPilot().Health} - 1");
            ...
            if (!target.UnitIsTrooperSquad() || !Mod.Config.DisableInjuriesForTroopers)
            {
                if (healthDamage > 0)
                {
                    ...
                    Mod.Log.Info?.Write($"Adding {healthDamage} to {target.DisplayName}");
                    target.GetPilot().StatCollection.ModifyStat<int>(hitInfo.attackerId, hitInfo.stackItemUID,
        ...
        public static void ApplyPilotSkillDamage(AbstractActor target, WeaponHitInfo hitInfo, int skillDamageHits, out string tooltipText)
        {
            ...
            Mod.Log.Info?.Write($"Applying {skillDamageHits} hits to pilot skills to: {target.DisplayName}");
            ...
            Mod.Log.Info?.Write($"  reducing piloting: -{pilotingMod}  gunnery: -{gunneryMod}  tactics: -{tacticsMod}  guts: -{gutsMod}"); ;
