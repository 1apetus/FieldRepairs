...
namespace FieldRepairs
{
    ...
    public class TurretRepairState : RepairState
    {
                ...
                while (!isResolved)
                {
                    ...
                    switch (damageType)
                    {
                        case DamageType.Skill:
                            PilotSkillHits++;
                            isResolved = true;
                            Mod.Log.Info?.Write($"  {i} is damageType: {damageType}.");
                            break;
        ...
        private static ComponentSummary AnalyzeComponents(Turret targetTurret)
        {
            ...
            {
                Mod.Log.Info?.Write($"Checking component: {mc.Name} / {mc.UIName} / {mc.Description.UIName}");
                ...
                {
                    Mod.Log.Info?.Write($"  - Skipping critical component: {mc.Description.UIName} in location: {(ChassisLocations)mc.Location}");
                }
                else if (isBlacklisted)
                {
                    Mod.Log.Info?.Write($"  - Skipping blacklisted component: {mc.Description.UIName}");
                }
                else if (mc.componentType == ComponentType.AmmunitionBox)
                {
                    Mod.Log.Info?.Write($"  - Found ammoBox: {mc.Description.UIName}");
                    compSummary.AmmoBoxes.Add((AmmunitionBox)mc);
                }
                else if (mc.componentType == ComponentType.HeatSink)
                {
                    Mod.Log.Info?.Write($"  - Found heatSink: {mc.Description.UIName}");
                    compSummary.HeatSinks.Add(mc);
                }
                else if (mc.componentType == ComponentType.Weapon)
                {
                    Mod.Log.Info?.Write($"  - Found weapon: {mc.Description.UIName}");
                    compSummary.Weapons.Add(mc);
                    // Check weapons for volatile? If we don't apply effects, do we care?                    
                    if (mc.componentDef.CanExplode)
                    {
                        Mod.Log.Info?.Write($"      weapon has component explosion: CanExplode");
                    }
                }
                else
                {
                    Mod.Log.Info?.Write($"  - Found component: {mc.Description.UIName} in location: {mc.Location}");
